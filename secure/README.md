...
| Inline `style` with a `url(...)` — object `style={{ background: 'url(https://attacker/…)' }}` **or** string `style="background:url(https://attacker/…)"` | **unblocked** | The renderer applies the declared value as-is; the CSS engine fetches the URL when it paints. `url()` is **not** parsed out of style values in either form. (The object form is additionally rebuilt with a null prototype and read via own-data descriptors only — that hardening defends against prototype-pollution / accessor side effects, NOT against author-declared `url()`.) Block with a `style-src`/`img-src` CSP. |
| `<a href="https://attacker" target="_blank">` | **blocked by default / mitigated when opted in** | `target` is OFF the default allowlist (dropped entirely). If a host opts it in via `allowedAttrs: ['target']`, only `_self`/`_blank` values survive (`_top`/`_parent` dropped) and `_blank` is force-annotated `rel="noopener noreferrer"`, so `window.opener` is not leaked. Cross-origin navigation on user click is still possible. |
