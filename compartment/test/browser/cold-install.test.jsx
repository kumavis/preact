...
// This file deliberately NEVER calls `secureRender` before its
// assertions, so the `preact/secure` module is "cold" (its option
// hooks were never armed by a `secureRender()` call). It is a
...
//   Previously `install()` ran ONLY from `secureRender()`. A host
//   that wired `confineComponent` but mounted a Confined via plain
//   `render()` — never calling `secureRender()` — got NO sanitization
...
// of any `secureRender` import/call before the assertions, and do not
...
describe('preact/compartment cold-module fail-fast', () => {
	...
	it('Confined via plain render throws even with no prior secureRender (cold module)', () => {
